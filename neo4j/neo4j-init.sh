...
if [ ! -f /tmp/neo4j-import-done.flag ]; then
    ...
    /var/lib/neo4j/bin/neo4j-admin import --multiline-fields=true --skip-duplicatate-nodes=true --database graph.db --delimiter '\t' --nodes /neo4j/graph_complex_all.neo.nodes --relationships /neo4j/graph_complex_all.neo.edges
