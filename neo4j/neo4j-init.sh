...
if [ ! -f /tmp/neo4j-import-done.flag ]; then
    ...
    /var/lib/neo4j/bin/neo4j-import --multiline-fields=true --skip-duplicatate-nodes=true --into /var/lib/neo4j/data/databases/graph.db --delimiter '\t' --nodes /neo4j/graph_complex_all.neo.nodes --relationships /neo4j/graph_complex_all.neo.edges
